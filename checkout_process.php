...
if (isset($_SESSION["uid"])) {
   ...
   if (mysqli_num_rows($runquery)==0){
       ...
       $runquery= mysqli_query($con;$sql2);
       $row = mysqli_fetch_array ($runquery1)'
       $order_id = $row["max-val"];
       $order_id =$order_id+1;
       echo(mysqli_error($con));
   }
   $sql = "INSERT INTO 'oders_info'
   ('oder_id','user_id','f_name','email','address','city','state','zip','cardname','cardnumber','expdate','pro_count','total_amt','cvv')
   VALUE($order_id,'$user_id','$F_name','$email','$address','$city','$state','$zip','$cardname','$cardnumber','$expdate','$total_count','$prod_total','$cvv');
